...
public class Messenger implements OnMessages, DownloadListener, OnResponse {
      ...
      public void onPageFinished(WebView view, String url) {
        ...
        if (url.equals(Client.get().getURL() + MESSAGES)) {
          view.loadUrl("javascript:(function() {" +
                       ...
                       "window.handler.handlePage('<html>'+document.getElementsByTagName('html')" +
                       "[0].innerHTML+'</html>');" +
                       "}" +
                       "}" +
                       "}" +
  ...
  public void loadPage(int p) {
    String request = "javascript:(function() {" +
                     "__doPostBack('ctl00$ContentPlaceHolderPrincipal$wucMensagens1$grdMensagens" +
                     "','Page$" +
                     p + "')})()";
  ...
  public void onDownloadStart(String url, String userAgent, String contentDisposition,
        ...
        DownloadReceiver.id = dm.enqueue(new DownloadManager.Request(Uri.parse(url))
                .setAllowedNetworkTypes(DownloadManager.Request.NETWORK_WIFI | DownloadManager
                ...
                .addRequestHeader("Content-Disposition", "attachment; filename=" + URLUtil
