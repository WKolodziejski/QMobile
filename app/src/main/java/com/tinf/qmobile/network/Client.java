...
public class Client {
  ...
  private void createRequest(int pg,
    ...
    if (!isValid) {
      ...
      addRequest(new StringRequest(method, URL + url, responseASCII -> {
        ...
        Resp r =
            ResponseParser.parseResponse(response, pg, this::callOnError, this::callOnAccessDenied);

        if (r == Resp.DENIED) {
          addToQueue(pg, url, year, period, method, form, notify, payload, onFinish);
          login();
          callOnAccessDenied(pg, getContext().getResources()
  ...
  public <T> void addRequest(Request<T> request,
    ...
    if (isConnected()) {
      ...
      onError(pg, new VolleyError(getContext().getResources()
  ...
  public void login() {
    ...
    executors.execute(() -> fetchParams(
        success -> addRequest(new StringRequest(POST, URL + VALIDA, responseASCII -> {
          ...
          Resp r = ResponseParser.parseResponse(response, PG_LOGIN, this::callOnError,
