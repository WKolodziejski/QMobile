...
public class PopUpHandler {
  ...
  public void handleLogin(String page) {
    ...
    if (body != null) {
      String title =
          body.getElementsByClass("subtitulo").first().text().replaceAll("\\\\n", "\n").trim();
      String message =
          body.getElementsByClass("conteudoTexto").first().text().replaceAll("\\\\n", "\n").trim();

      onEvent.onDialog(webView, title, message);
    } else if (requested) {
      Toast.makeText(getContext(), getContext().getString(R.string.dialog_no_alerts),
