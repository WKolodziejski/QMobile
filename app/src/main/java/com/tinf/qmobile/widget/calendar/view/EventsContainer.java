...
import com.tinf.qmobile.widget.calendar.view.comparators.EventComparator;
...
public class EventsContainer {
  ...
  private final Map<String, List<Events>> eventsByMonthAndYearMap = new HashMap<>();
  private final Comparator<Event> eventsComparator = new EventComparator();
  ...
  List<Events> getEventsForMonthAndYear(int month,
                                        int year) {
  ...
  void removeEvent(Event event) {
    ...
    if (eventsForMonthAndYear != null) {
      ...
      while (eventsForMonthYrItr.hasNext()) {
        ...
        int indexOfEvent = events.getEvents()
                                 .indexOf(event);
        if (indexOfEvent >= 0) {
          if (events.getEvents()
                    .size() == 1) {
