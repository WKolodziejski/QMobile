...
public class CompactCalendarView extends View {
  ...
  public CompactCalendarView(Context context,
    ...
    compactCalendarController =
        new CompactCalendarController(new Paint(), new OverScroller(getContext()),
                                      new Rect(), attrs, getContext(), Color.argb(255, 233, 84, 81),
                                      Color.argb(255, 64, 64, 64), Color.argb(255, 219, 219, 219),
   ...
   */
  public void setLocale(TimeZone timeZone,
                        Locale locale) {
  ...
  on the length of each day name.
  ...
  /**
   * see {@link #addEvent(Event, boolean)} when adding single events to control if calendar
   * should redraw
  ...
  /**
   * Fetches the events for the month of the epochMillis passed in and returns a sorted list of
   * events
   *
   * @param epochMillis
   * @return
  ...
  /**
   * see {@link #removeEvent(Event, boolean)} when removing single events to control if
   * calendar should redraw
  ...
  private void checkTargetHeight() {
    if (compactCalendarController.getTargetHeight() <= 0) {
      throw new IllegalStateException(
          "Target height must be set in xml properties in order to expand/collapse " +
          "CompactCalendar.");
    }
  }
  ...
  /**
   * Moves the calendar to the left. This will show the previous month when
   * {@link #setIsRtl(boolean)}
  ...
  @Override
  protected void onMeasure(int parentWidth,
                           int parentHeight) {
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    // on touch action finished (CANCEL or UP), we re-allow the parent container to intercept
    // touch events (scroll inside ViewPager + RecyclerView issue #82)
    if ((event.getAction() == MotionEvent.ACTION_CANCEL ||
         event.getAction() == MotionEvent.ACTION_UP) && horizontalScrollEnabled) {
