...
public class ScheduleViewHome extends View {
  ...
  private final GestureDetector.SimpleOnGestureListener mGestureListener =
      new GestureDetector.SimpleOnGestureListener() {
          ...
          super.onLongPress(e);
            ...
            /*if (mEventLongPressListener != null && mEventRects != null) {
                ...
                for (EventRect event : reversedEventRects) {
                    if (event.rectF != null && e.getX() > event.rectF.left && e.getX() < event
                    .rectF.right && e.getY
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
          ...
          if (mEventRects != null && mEventClickListener != null) {
            ...
            for (EventRect eventRect : reversedEventRects) {
              if (!mNewEventIdentifier.equals(eventRect.event.getIdentifier()) &&
                  eventRect.rectF != null && e
                                                 .getX() > eventRect.rectF.left &&
                  e.getX() < eventRect.rectF.right && e.getY() >
                                                      eventRect.rectF
                                                          .top &&
                  e.getY() < eventRect.rectF.bottom) {
          ...
          if (mAddEventClickListener != null && mNewEventRect != null &&
              mNewEventRect.rectF != null &&
              mNewEventRect.rectF.contains(x, y)) {
            mAddEventClickListener.onAddEventClicked(mNewEventRect.event.getStartTime(),
                                                     mNewEventRect.event
                                                         .getEndTime());
            return super.onSingleTapConfirmed(e);
          }
          ...
          if ((mEmptyViewClickListener != null || mAddEventClickListener != null)) {
            ...
            if (selectedTime != null) {
              ...
              if (mAddEventClickListener != null) {
                ...
                selectedTime.addMinutes(
                    mod < Math.ceil(mNewEventTimeResolutionInMinutes / 2) ? -mod :
                ...
                int maxMinutes =
                    (mMaxTime - selectedTime.getHour()) * 60 - selectedTime.getMinute() - 1;
                ...
                WeekViewEvent newEvent =
                    new WeekViewEvent(mNewEventIdentifier, "", null, selectedTime,
                                      endTime);
                ...
                float left =
                    mWidthPerDay * WeekViewUtil.daysBetween(getFirstVisibleDay(), selectedTime
                        .getDay());
  ...
  public ScheduleViewHome(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    try {
      ...
      mNewEventLengthInMinutes =
          a.getInt(me.jlurena.revolvingweekview.R.styleable.WeekView_newEventLengthInMinutes,
                   mNewEventLengthInMinutes);
      mNewEventTimeResolutionInMinutes =
          a.getInt(
              me.jlurena.revolvingweekview.R.styleable.WeekView_newEventTimeResolutionInMinutes,
              mNewEventTimeResolutionInMinutes);
      mEventPadding =
  ...
  private void drawEmptyImage(RectF rect, Canvas canvas, float originalTop, float originalLeft) {
    ...
    canvas.drawBitmap(icon, originalLeft + (rect.width() - icon.getWidth()) / 2,
                      originalTop + (rect.height() -
   ...
   * @param canvas       The canvas to draw upon.
   * @param originalTop  The original top position of the rectangle. The rectangle may have
   *                     some of its portion
   *                     outside of the visible area.
   * @param originalLeft The original left position of the rectangle. The rectangle may have
   *                     some of its portion
   *                     outside of the visible area.
   */
      ...
      if (availableHeight >= lineHeight) {
        ...
        do {
          ...
          if (!mNewEventIdentifier.equals(event.getIdentifier())) {
            textLayout =
                new StaticLayout(TextUtils.ellipsize(bob, mEventTextPaint, availableLineCount *
                                                                           availableWidth,
                                                     TextUtils.TruncateAt.END), mEventTextPaint,
                                 (int) (rect.right -
                                        originalLeft
                                        - mEventPadding * 2), Layout.Alignment.ALIGN_NORMAL, 1.0f,
                                 0.0f, false);
          }
  ...
  private void drawHeaderRowAndEvents(Canvas canvas) {
    ...
    if (mIsFirstDraw) {
      ...
      // week.
      if (getRealNumberOfVisibleDays() >= 7 && mHomeDay != mFirstDayOfWeek &&
          mShowFirstDayOfWeekFirst) {
    ...
    for (int dayNumber = leftDaysWithGaps + 1;
      ...
      if (mWidthPerDay + startPixel - start > 0) {
        if (mShowDistinctPastFutureColor) {
          ...
          Paint futurePaint =
              isWeekend && mShowDistinctWeekendColor ? mFutureWeekendBackgroundPaint :
              mFutureBackgroundPaint;
          float startY = mCurrentOrigin.y;

          if (isToday) {
            float beforeNow = (now.getHour() - mMinTime + now.getMinute() / 60.0f) * mHourHeight;
            canvas.drawRect(start, startY, startPixel + mWidthPerDay, startY + beforeNow,
                            pastPaint);
            canvas.drawRect(start, startY + beforeNow, startPixel + mWidthPerDay, getHeight(),
                            futurePaint);
      ...
      for (int hourNumber = mMinTime; hourNumber < mMaxTime; hourNumber++) {
        float top = +mCurrentOrigin.y + mHourHeight * (hourNumber -
                                                       mMinTime);
        if (top > -mHourSeparatorHeight && top < getHeight() &&
    ...
    for (int dayNumber = leftDaysWithGaps + 1;
      ...
      //canvas.drawText(dayLabel, startPixel + mWidthPerDay / 2, mHeaderTextHeight +
      // mHeaderRowPadding, isToday ?
      ...
      for (EventRect eventRect : mEventRects) {
        for (DayOfWeek day : days) {
          if (eventRect.event.getStartTime().getDay() == day && !eventRect.event.isAllDay()) {
            ...
            if (endTime == null ||
                getPassedMinutesInDay(endTime) < getPassedMinutesInDay(eventRect.event
                                                                           .getEndTime())) {
  ...
  public boolean dayIsValid(DayOfWeek day) {
    return (mMinDay == null || day.compareTo(mMinDay) >= 0) &&
           (mMaxDay == null || day.compareTo(mMaxDay) <= 0);
  }
  ...
  public interface EmptyViewClickListener {
     ...
     *
     * @param day: {@link DayTime} object set with the day and time of the clicked position
     *             on the view.
     */
  ...
  public interface EmptyViewLongPressListener {
    /**
     * Similar to {@link me.jlurena.revolvingweekview.WeekView.EmptyViewClickListener} but
     * with long press.
     *
     * @param time: {@link DayTime} object set with the day and time of the long pressed position
     *              on the view.
     */
  ...
  public interface EventLongPressListener {
    /**
     * Similar to {@link me.jlurena.revolvingweekview.WeekView.EventClickListener} but with a
     * long press.
     *
     * @param event:     event clicked.
     * @param eventRect: view containing the clicked event.
