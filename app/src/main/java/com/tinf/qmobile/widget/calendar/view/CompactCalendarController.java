...
import static com.tinf.qmobile.widget.calendar.view.CompactCalendarView.FILL_LARGE_INDICATOR;
import static com.tinf.qmobile.widget.calendar.view.CompactCalendarView.NO_FILL_LARGE_INDICATOR;
import static com.tinf.qmobile.widget.calendar.view.CompactCalendarView.SMALL_INDICATOR;
...
class CompactCalendarController {
  ...
  private void loadAttributes(AttributeSet attrs,
    ...
    TypedArray typedArray = context.getTheme()
                                   .obtainStyledAttributes(attrs, R.styleable.CompactCalendarView,
                                                           0, 0);

    int colorSurface =
        ColorsUtils.getColor(context, com.google.android.material.R.attr.colorSurfaceContainer);
    ...
    currentDayBackgroundColor = colorRoles.getAccentContainer();
    currentDayTextColor = colorRoles.getOnAccentContainer();

    currentSelectedDayBackgroundColor = colorRoles.getAccent();
    currentSelectedDayTextColor = colorRoles.getOnAccent();

    calenderTextColor = colorText;
    otherMonthDaysTextColor = colorText;

    calenderBackgroundColor = colorSurface;
    multiEventIndicatorColor = colorSurface;

    textSize = typedArray.getDimensionPixelSize(
        R.styleable.CompactCalendarView_compactCalendarTextSize,
        (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, textSize,
                                        context.getResources()
                                               .getDisplayMetrics()));
    targetHeight = typedArray.getDimensionPixelSize(
        R.styleable.CompactCalendarView_compactCalendarTargetHeight,
        (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, targetHeight,
                                        context.getResources()
                                               .getDisplayMetrics()));
    eventIndicatorStyle =
        typedArray.getInt(R.styleable.CompactCalendarView_compactCalendarEventIndicatorStyle,
                          SMALL_INDICATOR);
  ...
  private void initScreenDensityRelatedValues(Context context) {
    if (context != null) {
      screenDensity = context.getResources()
  ...
  private void setCalenderToFirstDayOfMonth(Calendar calendarWithFirstDayOfMonth,
                                            Date currentDate,
                                            int scrollOffset,
                                            int monthOffset) {
  ...
  void setFirstDayOfWeek(int day) {
    if (day < 1 || day > 7) {
      throw new IllegalArgumentException(
          "Day must be an int between 1 and 7 or DAY_OF_WEEK from Java Calendar class. For more " +
          "information please see Calendar.DAY_OF_WEEK.");
    }
  ...
  void setLocale(TimeZone timeZone,
                 Locale locale) {
    if (locale == null) {
  ...
  void setUseWeekDayAbbreviation(boolean useThreeLetterAbbreviation) {
    ...
    this.dayColumnNames =
        WeekUtils.getWeekdayNames(locale, firstDayOfWeekToDraw, this.useThreeLetterAbbreviation);
  }

  void setDayColumnNames(String[] dayColumnNames) {
    if (dayColumnNames == null || dayColumnNames.length != 7) {
      throw new IllegalArgumentException(
          "Column names cannot be null and must contain a value for each day of the week");
    }
  ...
  void onMeasure(int width,
    ...
    bigCircleIndicatorRadius =
        shouldDrawIndicatorsBelowSelectedDays && eventIndicatorStyle == SMALL_INDICATOR ?
  ...
  private float getInterpolatedBigCircleIndicator() {
    ...
    float x = (x1 + textSizeRect.height()) /
              2f; // pick a point which is almost half way through heightPerDay and textSizeRect
  ...
  void onSingleTapUp(MotionEvent e) {
    ...
    int dayColumn =
        Math.round((paddingLeft + e.getX() - paddingWidth - paddingRight) / widthPerDay);
    int dayRow = Math.round((e.getY() - paddingHeight) / heightPerDay);

    setCalenderToFirstDayOfMonth(calendarWithFirstDayOfMonth, currentDate, monthsScrolledSoFar(),
                                 0);
  ...
  private void snapBackScroller() {
    float remainingScrollAfterFingerLifted1 =
        (accumulatedScrollOffset.x - (monthsScrolledSoFar * width));
    scroller.startScroll((int) accumulatedScrollOffset.x, 0,
  ...
  private void performScroll() {
    ...
    scroller.startScroll((int) accumulatedScrollOffset.x, 0,
                         ...
                         (int) (Math.abs((int) (remainingScrollAfterFingerLifted)) / (float) width *
                                ANIMATION_SCREEN_SET_DURATION_MILLIS));
  }
  ...
  void drawEvents(Canvas canvas,
                  Calendar currentMonthToDrawCalender,
                  int offset) {
    ...
    List<Events> uniqEvents = eventsContainer.getEventsForMonthAndYear(currentMonth,
                                                                       currentMonthToDrawCalender.get(
                                                                           Calendar.YEAR));
    ...
    if (uniqEvents != null) {
      for (int i = 0; i < uniqEvents.size(); i++) {
        ...
        float xPosition =
            widthPerDay * dayOfWeek + paddingWidth + paddingLeft + accumulatedScrollOffset.x +
            offset - paddingRight;
        float yPosition = weekNumberForMonth * heightPerDay + paddingHeight;

        if (((animationStatus == EXPOSE_CALENDAR_ANIMATION ||
              animationStatus == ANIMATE_INDICATORS) && xPosition >= growFactor) ||
        ...
        boolean isSameDayAsCurrentDay =
            shouldDrawCurrentDayCircle && (todayDayOfMonth == dayOfMonth) &&
        ...
        if (shouldDrawIndicatorsBelowSelectedDays ||
            (!shouldDrawIndicatorsBelowSelectedDays && !isSameDayAsCurrentDay &&
          ...
          if (eventIndicatorStyle == FILL_LARGE_INDICATOR ||
            ...
            if (shouldDrawIndicatorsBelowSelectedDays &&
                (isSameDayAsCurrentDay || isCurrentSelectedDay)) {
              yPosition += indicatorOffset;
            }
  ...
  private void drawSingleEvent(Canvas canvas,
                               float xPosition,
                               float yPosition,
  ...
  private void drawTwoEvents(Canvas canvas,
                             float xPosition,
                             float yPosition,
                             List<Event> eventsList) {
    //draw fist event just left of center
    drawEventIndicatorCircle(canvas, xPosition + (xIndicatorOffset * -1), yPosition,
                             eventsList.get(0)
                                       .getColor());
  ...
  //draw 2 eventsByMonthAndYearMap followed by plus indicator to show there are more than 2
  // eventsByMonthAndYearMap
  private void drawEventsWithPlus(Canvas canvas,
                                  float xPosition,
                                  float yPosition,
                                  List<Event> eventsList) {
    // k = size() - 1, but since we don't want to draw more than 2 indicators, we just stop after
    // 2 iterations so we can just hard k = -2 instead
    // we can use the below loop to draw arbitrary eventsByMonthAndYearMap based on the current
    // screen size, for example, larger screens should be able to
    // display more than 2 evens before displaying plus indicator, but don't draw more than 3
    // indicators for now
    for (int j = 0, k = -2; j < 3; j++, k += 2) {
      ...
      if (j == 2) {
        ...
        canvas.drawLine(xStartPosition - smallIndicatorRadius, yPosition,
                        xStartPosition + smallIndicatorRadius, yPosition, dayPaint);
        canvas.drawLine(xStartPosition, yPosition - smallIndicatorRadius, xStartPosition,
                        yPosition + smallIndicatorRadius, dayPaint);
  ...
  void drawMonth(Canvas canvas,
    ...
    for (int dayColumn = 0, colDirection = isRtl ? 6 : 0, dayRow = 0; dayColumn <= 6; dayRow++) {
      ...
      float xPosition =
          widthPerDay * dayColumn + paddingWidth + paddingLeft + accumulatedScrollOffset.x +
          offset - paddingRight;
      ...
      if (dayRow == 0) {
        ...
        if (currentCalender.get(Calendar.DAY_OF_MONTH) == day && isSameMonthAsCurrentCalendar &&
          ...
          drawDayCircleIndicator(currentDayIndicatorStyle, canvas, xPosition, yPosition,
        ...
        if (day <= 0) {
          if (displayOtherMonthDays) {
            ...
            canvas.drawText(String.valueOf(maximumPreviousMonthDay + day), xPosition, yPosition,
                            dayPaint);
          }
  ...
  private void drawDayCircleIndicator(int indicatorStyle,
                                      Canvas canvas,
                                      float x,
                                      float y,
                                      int color) {
  ...
  private void drawEventIndicatorCircle(Canvas canvas,
                                        float x,
                                        float y,
                                        int color) {
    ColorRoles colorRoles = ColorsUtils.harmonizeWithPrimary(context, color);

    dayPaint.setColor(colorRoles.getAccentContainer());
    if (eventIndicatorStyle == SMALL_INDICATOR) {
      ...
      dayPaint.setStyle(Paint.Style.STROKE);
      drawDayCircleIndicator(NO_FILL_LARGE_INDICATOR, canvas, x, y,
                             colorRoles.getAccentContainer());
    } else if (eventIndicatorStyle == FILL_LARGE_INDICATOR) {
      drawDayCircleIndicator(FILL_LARGE_INDICATOR, canvas, x, y, colorRoles.getAccentContainer());
    }
  }
