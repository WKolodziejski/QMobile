...
public class ReportAdapter extends AbstractTableAdapter<String, Matter, String>
  ...
  public ReportAdapter(Context context, TableView tableView, JournalEmptyBinding empty) {
    ...
    tableView.setTableViewListener(new ITableViewListener() {
      ...
      public void onCellClicked(
          @NonNull
          RecyclerView.ViewHolder cellView, int column, int row) {
      ...
      @Override
      public void onColumnHeaderClicked(
          @NonNull
          RecyclerView.ViewHolder columnHeaderView, int column) {
        tableView.getSelectionHandler().clearSelection();
      }

      @Override
      public void onColumnHeaderDoubleClicked(
          @NonNull
          RecyclerView.ViewHolder columnHeaderView, int column) {
      ...
      @Override
      public void onRowHeaderClicked(
          @NonNull
          RecyclerView.ViewHolder rowHeaderView, int row) {
        tableView.getSelectionHandler().clearSelection();
      }

      @Override
      public void onRowHeaderDoubleClicked(
          @NonNull
          RecyclerView.ViewHolder rowHeaderView, int row) {
      }

      @Override
      public void onRowHeaderLongPressed(
          @NonNull
          RecyclerView.ViewHolder rowHeaderView, int row) {
      }
    });
  ...
  private void loadList() {
    ...
    if (!matters.isEmpty()) {
      ...
      for (int i = 0; i < columns; i++) {
        ...
        if (!period.isSub_())
          columnHeader.add(
              context.getResources().getString(R.string.report_absences) + " " + period.getTitle());
      }
  ...
  public AbstractViewHolder onCreateColumnHeaderViewHolder(ViewGroup parent, int viewType) {
    if (viewType == 0 || viewType == columnHeader.size() - 1)
      return new TableColumnMatterHeaderViewHolder(LayoutInflater.from(context)
                                                                 .inflate(
                                                                     R.layout.table_column_matter,
                                                                     parent, false));

    return new TableColumnHeaderViewHolder(LayoutInflater.from(context)
                                                         .inflate(R.layout.table_column, parent,
                                                                  false));
  }

  @Override
  public void onBindColumnHeaderViewHolder(AbstractViewHolder holder, String columnHeaderItemModel,
                                           int c) {
    try {
  ...
  @Override
  public void onBindRowHeaderViewHolder(AbstractViewHolder holder, Matter rowHeaderItemModel,
                                        int r) {
    try {
