  ...
  public static Client.Resp parseResponse(String response,
    ...
    if (strong != null) {
      ...
      if (s.contains("negado") || s.contains("Negado")) {
        ...
        if (msg.contains("inativo")) {
          UserUtils.clearInfo();
          onAccessDenied.onError(PG_ACCESS_DENIED, msg);
    ...
    if (p != null) {
      if (p.text()
           .contains("inacess") || p.text()
                                    .contains("Banco")) {
        onError.onError(PG_LOGIN, getContext().getResources()
    ...
    if (form != null) {
      if (form.text()
        ...
        String msg = form.text()
                         .replaceAll("\\\\n", "\n")
                         .trim();
        onAccessDenied.onError(PG_UPDATE, msg);
    ...
    if (quest != null) {
      if (quest.text()
        ...
        onAccessDenied.onError(PG_QUEST, msg);
        return Client.Resp.QUEST;
      }

      if (quest.text()
               .contains("Altera")) {
        onAccessDenied.onError(PG_REGISTRATION, "");
    ...
    if (title != null && title.text()
                              .contains("Erro")) {
      onAccessDenied.onError(pg, getContext().getString(R.string.client_error));
