...
public class JournalParser extends BaseParser {
  ...
  public JournalParser(int page, int year, int period, boolean notify, OnFinish onFinish,
                       OnError onError) {
...
//            String cGiven = trs.get(2).getElementsByTag("td").get(1).text().trim();
      ...
      try {
        matter = matterBox.query()
                          .equal(Matter_.description_,
                                 StringUtils.stripAccents(description), CASE_INSENSITIVE)
                          .and()
      ...
      if (matter == null) {
        ...
        for (Matter m : matters)
          if (StringUtils.containsIgnoreCase(m.getDescription_(),
                                             StringUtils.stripAccents(description)))
            matter = m;
      }

      if (matter == null) {
        matter = new Matter(StringUtils.stripAccents(description), colors.getColor(), hours,
                            classesTotal, year, period);
        isFirstParse = true;
      }
      ...
      while (content != null && content.child(0).childrenSize() > 0 &&
             content.child(0).child(0).is("div")) {
        ...
        if (periodCount <= matter.periods.size() - 1) {
          ...
          if (periodTitle.contains("Exame") || periodTitle.contains("Reavalia") ||
              periodTitle.contains("Final") || periodTitle.contains("Conceito")) {
            period.setSub();
          }
        }
...
//                    String gradeString = formatJournal(grades.eq(j).first(), HeaderType.GRADE);
          ...
          if (grades.eq(j).first().child(3).text().contains(HeaderType.MAX.txt)) {
            ...
            maxString = formatGrade(
                formatNumber(maxString.substring(maxString.indexOf(" ")).trim()));
            gradeString =
                formatGrade(formatNumber(grades.eq(j).first().child(4).text()));
          } else {
            maxString = "";
            gradeString =
...
//                    .text()));
          ...
          try {
            QueryBuilder<Journal> builder = journalBox.query()
                                                      .equal(Journal_.title, title,
                                                             CASE_INSENSITIVE).and()
                                                      .equal(Journal_.type_, type).and()
                                                      .between(Journal_.startTime, date,
                                                               date).and()
                                                      .between(Journal_.weight_, weight,
                                                               weight).and()
          ...
          if (search == null) {
            Journal newJournal =
                new Journal(title, grade, weight, max, date, type, period, matter,
                            isFirstParse);
  ...
  private void notifyNewGrade(Journal journal) {
    ...
    NotificationUtils.show(
        String.format(Locale.getDefault(),
                      App.getContext()
                         .getResources()
                         .getString(R.string.notification_journal_title),
                      journal.getType(),
  ...
  private void notifyGradeChanged(Journal journal) {
    Intent intent = new Intent(App.getContext(), EventViewActivity.class);
    intent.putExtra("ID", journal.id);
    intent.putExtra("TYPE", JOURNAL);
    ...
    NotificationUtils.show(
        String.format(Locale.getDefault(),
                      App.getContext()
                         .getResources()
                         .getString(R.string.notification_grade_title)),
        journal.getTitle(),
        JOURNAL, (int) journal.id, intent);
  }
