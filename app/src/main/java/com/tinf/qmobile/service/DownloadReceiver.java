...
public class DownloadReceiver extends BroadcastReceiver {
  ...
  private static void openFile(Uri uri) {
    ...
    try {
      ...
      Toast.makeText(getContext(), getContext().getResources().getString(R.string.text_no_handler),
                     Toast.LENGTH_LONG).show();
    }
  }
  ...
  public static long download(Context context, String url, String title, String path) {
    try {
      if (Client.isConnected()) {
        ...
        Toast.makeText(context, context.getResources().getString(R.string.materiais_downloading),
                       Toast.LENGTH_SHORT).show();
        return id;
      } else {
        Toast.makeText(context, context.getResources().getString(R.string.client_no_connection),
                       Toast.LENGTH_SHORT).show();
        return 0;
      }
    } catch (Exception e) {
      ...
      Toast.makeText(context, context.getResources().getString(R.string.client_error),
                     Toast.LENGTH_SHORT).show();
    }
    return 0;
  }
  ...
  public static void downloadImage(Context context, String cod) {
    try {
      if (Client.isConnected()) {
        ...
        DownloadManager dm = (DownloadManager) context.getSystemService(DOWNLOAD_SERVICE);
        id = dm.enqueue(new DownloadManager.Request(
            Uri.parse((Client.get().getURL() + INDEX + "1025&tipo=0&COD=" + cod)))
                            .setAllowedNetworkTypes(DownloadManager.Request.NETWORK_WIFI |
                                                    DownloadManager.Request.NETWORK_MOBILE)
                            .setNotificationVisibility(
                                DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED)
